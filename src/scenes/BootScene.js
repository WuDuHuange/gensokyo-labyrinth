...
export default class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    if (SPRITE_CONFIG.useCustomSprite) {
      ...
      this.load.image('slowFairy', 'assets/sprites/slowFairy.png');
      this.load.image('normalFairy', 'assets/sprites/normalFairy.png');
      this.load.image('fastFairy', 'assets/sprites/fastFairy.png');
  ...
  /**
   * 创建临时精灵图（用于开发阶段）
   * 如果已经加载了自定义精灵，则跳过对应的生成
   */
  createTempSprites() {
    const useCustom = SPRITE_CONFIG.useCustomSprite;

    // 创建玩家精灵（灵梦 - 红白配色）- 仅在未加载自定义图时生成
    if (!useCustom || !this.textures.exists('player')) {
    ...
    // 创建慢速妖精（蓝色）
    if (!useCustom || !this.textures.exists('slowFairy')) {
    ...
    // 创建普通妖精（绿色）
    if (!useCustom || !this.textures.exists('normalFairy')) {
    ...
    // 创建快速妖精（黄色）
    if (!useCustom || !this.textures.exists('fastFairy')) {
    ...
    // 创建弹幕妖精（紫色）
    if (!useCustom || !this.textures.exists('danmakuFairy')) {
    ...
    rockGraphics.fillStyle(0x666677);
    rockGraphics.fillRoundedRect(4, 8, 24, 20, 4);
    rockGraphics.fillStyle(0x555566);
    rockGraphics.fillRect(8, 10, 8, 6);
    rockGraphics.fillRect(18, 14, 6, 8);
    rockGraphics.generateTexture('rock', 32, 32);
    rockGraphics.destroy();

    // 创建地刺陷阱贴图
    const spikeGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    spikeGraphics.fillStyle(0x444455);
    spikeGraphics.fillTriangle(6, 28, 10, 12, 14, 28);
    spikeGraphics.fillTriangle(14, 28, 18, 8, 22, 28);
    spikeGraphics.fillTriangle(22, 28, 26, 14, 30, 28);
    spikeGraphics.generateTexture('spike', 32, 32);
    spikeGraphics.destroy();

    // 创建传送阵贴图
    const portalGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    portalGraphics.lineStyle(2, 0x9966ff, 0.8);
    portalGraphics.strokeCircle(16, 16, 12);
    portalGraphics.lineStyle(1, 0xcc99ff, 0.6);
    portalGraphics.strokeCircle(16, 16, 8);
    portalGraphics.fillStyle(0x6633cc, 0.4);
    portalGraphics.fillCircle(16, 16, 6);
    portalGraphics.generateTexture('portal', 32, 32);
    portalGraphics.destroy();

    // 创建秘传书贴图
    const bookGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    bookGraphics.fillStyle(0x8b4513); // 棕色封面
    bookGraphics.fillRoundedRect(6, 4, 20, 24, 2);
    bookGraphics.fillStyle(0xfff8dc); // 米色书页
    bookGraphics.fillRect(8, 6, 16, 20);
    bookGraphics.fillStyle(0xdaa520); // 金色装饰
    bookGraphics.fillRect(6, 14, 20, 2);
    bookGraphics.fillStyle(0xff6b6b); // 红色书签
    bookGraphics.fillRect(22, 4, 3, 12);
    bookGraphics.generateTexture('book', 32, 32);
    bookGraphics.destroy();

    // 创建御守贴图（护符形状）
    const omamoriGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    omamoriGraphics.fillStyle(0xcc3333); // 红色底
    omamoriGraphics.fillRoundedRect(8, 2, 16, 22, 3);
    omamoriGraphics.fillStyle(0xffcc00); // 金色装饰
    omamoriGraphics.fillRect(14, 4, 4, 4);
    omamoriGraphics.fillStyle(0xffffff); // 白色绳结
    omamoriGraphics.fillCircle(16, 26, 3);
    omamoriGraphics.generateTexture('omamori', 32, 32);
    omamoriGraphics.destroy();

    // 创建勾玉贴图（绿色）
    const magatamaGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    magatamaGraphics.fillStyle(0x33cc66); // 绿色
    magatamaGraphics.fillCircle(16, 12, 8);
    magatamaGraphics.fillStyle(0x33cc66);
    magatamaGraphics.fillTriangle(16, 20, 10, 10, 22, 10); // 尾巴
    magatamaGraphics.fillStyle(0xffffff);
    magatamaGraphics.fillCircle(16, 10, 3); // 眼睛
    magatamaGraphics.generateTexture('magatama', 32, 32);
    magatamaGraphics.destroy();

    // 创建蓝色勾玉（灵之勾玉）
    const magatamaBlueGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    magatamaBlueGraphics.fillStyle(0x3399ff);
    magatamaBlueGraphics.fillCircle(16, 12, 8);
    magatamaBlueGraphics.fillStyle(0x3399ff);
    magatamaBlueGraphics.fillTriangle(16, 20, 10, 10, 22, 10);
    magatamaBlueGraphics.fillStyle(0xffffff);
    magatamaBlueGraphics.fillCircle(16, 10, 3);
    magatamaBlueGraphics.generateTexture('magatama_blue', 32, 32);
    magatamaBlueGraphics.destroy();

    // 创建缎带贴图
    const ribbonGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    ribbonGraphics.fillStyle(0xff4444); // 红色缎带
    ribbonGraphics.fillRect(4, 12, 24, 8);
    ribbonGraphics.fillTriangle(4, 12, 4, 20, 0, 16);
    ribbonGraphics.fillTriangle(28, 12, 28, 20, 32, 16);
    ribbonGraphics.fillStyle(0xffcccc);
    ribbonGraphics.fillCircle(16, 16, 4);
    ribbonGraphics.generateTexture('ribbon', 32, 32);
    ribbonGraphics.destroy();

    // 创建神社贴图（�的鸟居形状）
    const shrineGraphics = this.make.graphics({ x: 0, y: 0, add: false });
    // 鸟居门柱
    shrineGraphics.fillStyle(0xcc3333);
    shrineGraphics.fillRect(6, 8, 4, 22);  // 左柱
    shrineGraphics.fillRect(22, 8, 4, 22); // 右柱
    // 鸟居横梁
    shrineGraphics.fillRect(2, 4, 28, 5);
    shrineGraphics.fillRect(4, 10, 24, 3);
    // 金色装饰
    shrineGraphics.fillStyle(0xffd700);
    shrineGraphics.fillRect(2, 4, 28, 2);
    shrineGraphics.generateTexture('shrine', 32, 32);
